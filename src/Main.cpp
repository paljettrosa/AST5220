...
int main(int argc, char **argv){
  ...
  if (Planck) {
    ...
    Perturbations pert(&cosmo, &rec, n_ell_Theta, n_ell_Theta_P, n_ell_Nu, lensing);
    pert.info();
    pert.solve(-18.0, 0.0, 1000, 0.00005/Constants.Mpc, 1.0/Constants.Mpc, 200); 
    ...
    power.info();
    power.solve(-18.0, 0.0, 3000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, false, true, false, true); //TODO: why so unstable when using k_max = 1.0?

    // Print equality-scale
    power.print_equality_scale();

    // Output power spectrum quantities
    std::vector<int> ells = {2, 3, 4, 15, 100, 350, 850, 1350, 1850}; //TODO: remove 2, 3 and 4
    for (auto ell: ells)
      power.output_transfer_functions(0.00005/Constants.Mpc, 0.3/Constants.Mpc, ell, "results/transfer_functions_ell" + std::to_string(ell) + ".txt");
    power.output_C_of_theta("results/C_of_theta.txt");
    power.output_C_ells("results/C_ells.txt");
    power.output_P_k(0.00005, 1.0, "results/P_k.txt", true);
    power.output_xi(1.0, 500.0, "results/xi.txt");

    // Compute individual contributions to the TT power spectrum 
    if (contrib) {
      // Sachs-Wolfe term
      pert.solve(-18.0, 0.0, 1000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, 200, true, false, false, false); 
      PowerSpectrum power_SW(&cosmo, &rec, &pert, A_s, n_s, kpivot_Mpc);
      power_SW.solve(-18.0, 0.0, 3000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, true); 
      power_SW.output_C_ells("results/C_ell_SW.txt", true);

      // Integrated Sachs-Wolfe term
      pert.solve(-18.0, 0.0, 1000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, 200, false, true, false, false); 
      PowerSpectrum power_ISW(&cosmo, &rec, &pert, A_s, n_s, kpivot_Mpc);
      power_ISW.solve(-18.0, 0.0, 3000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, true);
      power_ISW.output_C_ells("results/C_ell_ISW.txt", true);

      // Doppler term
      pert.solve(-18.0, 0.0, 1000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, 200, false, false, true, false); 
      PowerSpectrum power_Doppler(&cosmo, &rec, &pert, A_s, n_s, kpivot_Mpc);
      power_Doppler.solve(-18.0, 0.0, 3000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, true);
      power_Doppler.output_C_ells("results/C_ell_Doppler.txt", true);

      // Polarization term
      pert.solve(-18.0, 0.0, 1000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, 200, false, false, false, true); 
      PowerSpectrum power_pol(&cosmo, &rec, &pert, A_s, n_s, kpivot_Mpc);
      power_pol.solve(-18.0, 0.0, 3000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, true);
  ...
  if (toy) {
    ...
    // Milestone III
    Perturbations pert_toy(&cosmo_toy, &rec_toy, 8, 0, 0, false);
    pert_toy.solve(-18.0, 0.0, 1000, 0.00005/Constants.Mpc, 0.3/Constants.Mpc, 200);
    ...
    // Milestone IV
    PowerSpectrum power_toy(&cosmo_toy, &rec_toy, &pert_toy, 1e-9, n_s, kpivot_Mpc);
