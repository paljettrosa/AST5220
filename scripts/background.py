...
t_rm = Omega_r0**(3/2) / (2*H0*(1000/Mpc)*Omega_m0**2)
eta_rm = c/(H0*(1000/Mpc)) * np.sqrt(Omega_r0)/Omega_m0
print("Radiation-matter equality:")
print(f"x:     {x_rm:.3f}")
print(f"z:     {z_rm:.3f}")
print(f"t:     {t_rm/kyr:.3f} kyr")
print(f"eta/c: {eta_rm/c/Myr:.3f} Myr")
print(f"eta:   {eta_rm/Mpc:.3f} Mpc\n")

# Onset of acceleration
x_acc = np.log(Omega_m0/(2*Omega_Lambda0)) / 3
z_acc = (2*Omega_Lambda0/Omega_m0)**(1/3) - 1
t_acc = 1/(3*H0*(1000/Mpc)) * (np.sqrt(2/Omega_Lambda0) - Omega_r0**(3/2) / (2*Omega_m0**2))
eta_acc = c/(H0*(1000/Mpc)) * (2**(5/6)/(Omega_m0**(1/3)*Omega_Lambda0**(1/6)) - np.sqrt(Omega_r0)/Omega_m0)
print("Onset of acceleration:")
print(f"x:     {x_acc:.3f}")
print(f"z:     {z_acc:.3f}")
print(f"t:     {t_acc/Gyr:.3f} Gyr")
print(f"eta/c: {eta_acc/c/Gyr:.3f} Gyr")
print(f"eta:   {eta_acc/Gpc:.3f} Gpc\n")

# Matter-dark energy equality
x_mLambda = np.log(Omega_m0/Omega_Lambda0) / 3
z_mLambda = (Omega_Lambda0/Omega_m0)**(1/3) - 1
t_mLambda = 1/(3*H0*(1000/Mpc)) * (2/np.sqrt(Omega_Lambda0) - Omega_r0**(3/2) / (2*Omega_m0**2))
eta_mLambda = c/(H0*(1000/Mpc)) * (2/(Omega_m0**(1/3)*Omega_Lambda0**(1/6)) - np.sqrt(Omega_r0)/Omega_m0)
print("Matter-dark energy equality:")
print(f"x:     {x_mLambda:.3f}")
print(f"z:     {z_mLambda:.3f}")
print(f"t:     {t_mLambda/Gyr:.3f} Gyr")
print(f"eta/c: {eta_mLambda/c/Gyr:.3f} Gyr")
print(f"eta:   {eta_mLambda/Gpc:.3f} Gpc\n")

# Today
x_0 = 0
z_0 = 0
t_0 = 1/(H0*(1000/Mpc)*np.sqrt(Omega_Lambda0)) * (2/3 - np.log(Omega_m0/Omega_Lambda0)/3 - np.sqrt(Omega_Lambda0)*Omega_r0**(3/2)/(6*Omega_m0**2)) 
eta_0 = - c/(H0*(1000/Mpc)) * (1/(np.sqrt(Omega_Lambda0)) + np.sqrt(Omega_r0)/Omega_m0 - 3/(Omega_m0**(1/3)*Omega_Lambda0**(1/6)))
print("Today:")
print(f"x:     {x_0:.3f}")
print(f"z:     {z_0:.3f}")
print(f"t:     {t_0/Gyr:.3f} Gyr")
print(f"eta/c: {eta_0/c/Gyr:.3f} Gyr")
print(f"eta:   {eta_0/Gpc:.3f} Gpc\n")
