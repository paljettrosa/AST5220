...
% \end{align}
% where I have used that $\mathcal{H}=aH = \dot{a}$. \colorbox{Plum}{do more thorough? t as function of x instead?}
...
% \colorbox{Plum}{include this?}
% In an era dominated by a component with equation of state $w_i$ we have
% \begin{equation}
%   \eta_i(x) \approx \eta_{i-1}(x_{i,\text{start}}) + \int_{x_{i,\text{start}}}^{x}\frac{cdx'}{H_0\sqrt{\Omega_{i0}e^{-(1+3w_i)x'}}},
% \end{equation}
% where $x_{i,\text{start}}$ denotes the value of $x$ when the Universe first became dominated by component $i$, and $\eta_{i-1}(x_{i,\text{start}})$ is the (approximate) value of the conformal time, estimated using the expression for the previously dominating component. For radiation we simply have $x_{r,\text{start}}=-\infty$ and thus $\eta(x_{r,\text{start}})=0$, hence
% \begin{equation}
%   \eta_r(x) = \int_{{-\infty}}^{x}\frac{cdx'}{H_0\sqrt{\Omega_{r0}}e^{-x'}} = \frac{c}{H_0\sqrt{\Omega_{r0}}e^{-x}}.
% \end{equation}
% For matter it then follows
% \begin{align*}
%   \eta_m(x) &\approx \eta_r(x_{rm}) + \int_{x_{rm}}^{x}\frac{cdx'}{H_0\sqrt{\Omega_{m0}e^{-x'}}}, 
%   \\
%   &= \frac{c}{H_0}\frac{\sqrt{\Omega_{r0}}}{\Omega_{m0}}+\frac{2c}{H_0}\left[\frac{1}{\sqrt{\Omega_{m0}e^{-x}}} - \frac{\sqrt{\Omega_{r0}}}{\Omega_{m0}} \right],
%   \\
%   &= \frac{c}{H_0}\left[\frac{2}{\sqrt{\Omega_{m0}e^{-x}}} - \frac{\sqrt{\Omega_{r0}}}{\Omega_{m0}} \right], \numberthis
% \end{align*}
% and for dark energy we have
% \begin{align*}
%   \eta_\Lambda(x) &\approx \eta_m(x_{m\Lambda}) + \int_{x_{m\Lambda}}^{x}\frac{cdx'}{H_0\sqrt{\Omega_{\Lambda0}}e^{x'}}, 
%   \\
%   &= \frac{c}{H_0}\left[\frac{2}{\Omega_{m0}^{1/3}\Omega_{\Lambda0}^{1/6}} - \frac{\sqrt{\Omega_{r0}}}{\Omega_{m0}}\right] - \frac{c}{H_0}\left[\frac{1}{\sqrt{\Omega_{\Lambda0}}e^{x}} - \frac{1}{\Omega_{m0}^{1/3}\Omega_{\Lambda0}^{1/6}}\right],
%   \\
%   &= -\frac{c}{H_0}\left[\frac{1}{\sqrt{\Omega_{\Lambda0}}e^{x}} +\frac{\sqrt{\Omega_{r0}}}{\Omega_{m0}} - \frac{3}{\Omega_{m0}^{1/3}\Omega_{\Lambda0}^{1/6}} \right]. \numberthis
% \end{align*}
