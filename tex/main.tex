...
% $f$, which describes the number density of particles in a given region of phase space. In thermal equilibrium, it follows the Fermi-Dirac (plus sign, applies to fermions) and Bose-Einstein (minus sign, applies to bosons) distributions:
\begin{equation}
...
% In figure \ref{fig:X_e and n_e} I have plotted the free electron fraction, $X_e$ (left subplot), and the electron number density, $n_e$ (right subplot), as a function of $x$. We see that $X_e \lesssim 1.2$ in the beginning when the Universe is fully ionized, slightly larger than 1 since we also have Helium. \colorbox{Plum}{should I specify number?} Furthermore, around $x \approx -9$, doubly ionized Helium recombines to singly ionized Helium, while further recombination into neutral Helium occurs shortly afterward, at $x \approx -8$. At around $x\approx -7$ the free electron fraction rapidly drops as most electrons combine with protons to form neutral Hydrogen, proving Hydrogen recombination to be the dominant process. \colorbox{Plum}{bad to split paragraph here?}

% This fact is also evident from the electron number density $n_e$, which decreases around four orders of magnitude during Hydrogen recombination, while no deviation from the usual volume dilution is visible during Helium recombination. colorbox{Plum}{word different?} We see this at later times as well: although the free electron fraction does increase at the point of $\text{He}^{+}$ reionization, the overall change in the number density is negligible compared to the multiple orders of magnitude that it increases around the first reionization period.

% % Moreover, the dashed blue line indicates the expected recombination time based on the Saha approximation, which predicts recombination happening slightly earlier than in the full numerical solution, marked by the solid blue line. This discrepancy arises due to non-equilibrium effects included in the Peebles equation, as discussed in section \ref{subsubsec: II theory Peebles}.
...
% The Einstein equations relate the gravitational potentials to the density perturbations and the velocity divergence of different components in the Universe. 
...
% In the next section, I introduce the Boltzmann equation for photons and discuss how perturbations in the CMB temperature and momentum are generated, leading to the anisotropies we observe today.
...
% As the Universe expands and Compton scattering becomes inefficient, $\tau'$ rapidly drops and tight-coupling breaks down. Numerically, this occurs when the condition
% \begin{equation}
% \frac{1}{\tau'} \sim \text{few}
% \end{equation}
% is met. At this point, we smoothly switch from the tight-coupling approximation to evolving the full Boltzmann hierarchy, as described in the earlier sections. This transition is crucial for accurately capturing the onset of recombination, when photon decoupling allows anisotropies to develop and eventually form the primary CMB spectrum.
